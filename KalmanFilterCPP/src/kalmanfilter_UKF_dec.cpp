...
{
    state(2) = wrapAngle(state(2)); // Normalise PHI; -pi<PHI<+pi
    return state; 
} 

VectorXd normaliseLidarMeasurement(VectorXd meas)
{
    meas(1) = wrapAngle(meas(1)); // Normalise Orientetaion Angle 
    return meas;
}
// ----------------------------------------------------------------------- //
...
std::vector<VectorXd> generateSigmaPoints(VectorXd state, MatrixXd cov)
{
    ...
    int numStates = state.size(); // state size
    double kappa = 3.0; // kappa = 3
    double lambda = kappa - numStates; 

    // Cholesky Decomposition function usage
    MatrixXd sqrtCov = cov.llt().matrixL(); 
    sigmaPoints.push_back(state); // Added data end of the vector 
...
std::vector<double> generateSigmaWeights(unsigned int numStates)
{
    ...
    double w0 = lambda / (lambda+numStates); // First value of Weights vector
    double wi = 0.5/(numStates+lambda); // Other values of Weights vector.
...
VectorXd vehicleProcessModel(VectorXd aug_state, double psi_dot, double dt)
{
    ...
    double psi_dot_noise = aug_state(4); // with additive noise
    double accel_noise = aug_state(5); // with additive noise
...
VectorXd lidarMeasurementModel(VectorXd aug_state, double beaconX, double beaconY)
{
    ...
    double psi = aug_state(2);
    double V = aug_state(3);
    double beaconX_noise = aug_state(4); // with non-additive noise
    double beaconY_noise = aug_state(5); // with non-additive noise

    double r_hat = sqrt((beaconX - x)*(beaconX - x) + (beaconY - y)*(beaconY - y)) + beaconX_noise;
    double theta_hat = atan2((beaconY - y),(beaconX - x)) - psi + beaconY_noise;
